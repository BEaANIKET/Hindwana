...
const ProductCard = ({ item }) => {
  return (
    <View style={styles.card}>
      <Image source={{ uri: `https://socket.hindwana.com/public/Images/${item?.image}`}} style={styles.image} />
      <View style={styles.contentContainer}>
        <Text style={styles.title}>{item.Title}</Text>
        <Text style={styles.description}>{item.sdescription}</Text>
        <Text style={styles.price}>₹{item.dprice.toFixed(2)}</Text>
        ...
        <TouchableOpacity style={styles.buyButton} onPress={() => console.log('Add to cart:', item.id)}>
          ...
          {/* <AntDesign name="Bus" size={16} color="white" style={styles.cartIcon} /> */}
        </TouchableOpacity>
      </View>
    </View>
  );
...
const travel = () => { 
  ...
  const gettravel = async() => {
    try {
      const response = await axios.get("https://socket.hindwana.com/api/traveldata")
